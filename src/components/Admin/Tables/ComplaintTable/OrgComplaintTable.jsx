...
const OrgComplaintTable = ({ complaints }) => {
  ...
  const TableRow = memo(({ complaint }) => (
    <tr className="hover:bg-gray-50 transition-colors">
      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
        {complaint.organizationId?.name || 'N/A'}
      </td>
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
        {complaint.organizationId?.email || 'N/A'}
      </td>
...
OrgComplaintTable.propTypes = {
  complaints: PropTypes.arrayOf(
    PropTypes.shape({
      _id: PropTypes.string.isRequired,
      organizationId: PropTypes.shape({ name: PropTypes.string, email: PropTypes.string }),
...
export default OrgComplaintTable;
